...
namespace ii.AscendancyLib.Reader
{
	public class VocReader
	{
		...
		public List<(byte[], string)> Read(byte[] bytes, bool fallbackToRaw)
		{
			using var ms = new MemoryStream(bytes, writable: false);
			using var br = new BinaryReader(ms);
			return ReadInternal(br, fallbackToRaw, () => (new RawReader().Read(bytes), string.Empty));
		}
		...
		public List<(byte[], string)> Read(string sourceFile, bool fallbackToRaw)
		{
			...
			return ReadInternal(br, fallbackToRaw, () => (new RawReader().Read(sourceFile), string.Empty));
		}

		private List<(byte[], string)> ReadInternal(BinaryReader br, bool fallbackToRaw, Func<(byte[], string)> rawOnInvalidVoc)
		{
